...
from access_trades.models import by_symbols
...
def getSingleStockCandles(request):
    ...
    for each in allStockNames:
        ...
        elif currentPosition % 60 != 0:

            utiliites.runTradingBot(request, each)


            # CREATE BC-PEFORMANCE OBJECTS FROM ALL THE TRADES.
            all_trades_ = all_trades.objects.all()
            seperated_trades = seperate_trades_by_bc(all_trades_)
            create_bc_peformances(seperated_trades)

            # CREATE CD-PEFORMANCE OBJECTS FROM ALL THE TRADES.
            # cd = create_cd_peformances()
        print('complete')
